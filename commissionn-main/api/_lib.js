...
export const markup = async (request, response) => { const token = session(request)?.access_token; if (!token) return response.status(401).json({ error: 'Please sign in with Deriv first.' }); const { start_date: startDate, end_date: endDate } = request.query; if (!/^\d{4}-\d{2}-\d{2}$/.test(startDate) || !/^\d{4}-\d{2}-\d{2}$/.test(endDate) || startDate > endDate) return response.status(400).json({ error: 'Choose a valid date range.' }); try { const [statsResult, appsResult] = await Promise.allSettled([getMarkupStatistics({ accessToken: token, dateFrom: `${startDate} 00:00:00`, dateTo: `${endDate} 23:59:59` }), getApplicationList({ accessToken: token })]); if (statsResult.status === 'rejected') throw statsResult.reason; response.json({ ...statsResult.value, app_list: appsResult.status === 'fulfilled' ? appsResult.value.app_list || [] : [] }) } catch (error) { response.status(502).json({ error: error.message || 'Unable to reach Deriv markup statistics.' }) } }
